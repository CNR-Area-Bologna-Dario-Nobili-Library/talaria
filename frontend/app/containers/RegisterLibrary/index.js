...
import {
  ...
  requestLibrarySubjectOptionList,
  requestPostPublicLibrary,
  requestGetProjectsOptionList,
  requestGetInstitutionsByTypeByCountryOptionList,
  requestGetInstitutionTypeOptionList,
  requestGetlibraryProjectsOptionList,
  requestGetlibraryidentifierTypesOptionList,
} from './actions';
import {
  institutionsOptionListSelector,
  countriesOptionListSelector,
  librarySubjectOptionListSelector,
  projectsOptionListSelector,
  institutionsByTypeCountryOptionListSelector,
...
const ILL_REQUEST_PAYMENT =
  process.env.ILL_REQUEST_PAYMENT && process.env.ILL_REQUEST_PAYMENT == 'true'
    ? true
    : false;
const LIBRARY_DIFFERENT_PROFILES =
  process.env.LIBRARY_DIFFERENT_PROFILES &&
  process.env.LIBRARY_DIFFERENT_PROFILES == 'true'
    ? true
    : false;
...
const RegisterLibrary = props => {
  ...
  useEffect(() => {
    ...
    setData({
      ...data,
      ['profile_type']: basicProfile ? 1 : 2,
      ['order_profile_type']: 0,
    });
    setData({
      ...data,
      ['profile_type_name']: basicProfile ? 'Basic Profile' : 'Full Profile',
      ['order_profile_type_name']: 1,
    });
  ...
  useEffect(() => {
    ...
    setData({
      ...
      ['profile_type']: basicProfile ? 1 : 2,
      ['order_profile_type']: 0,
      ['profile_type_name']: basicProfile
        ? intl.formatMessage(messages['basicprofile'])
        : intl.formatMessage(messages['fullprofile']),
      ['order_profile_type_name']: 1,
    });
  }, [basicProfile]);
  ...
  useEffect(() => {
    if (sortingcount === 1) {
      console.log(
        JSON.stringify('new sorted array ' + JSON.stringify(itemsreport)),
      );
  ...
  useEffect(() => {
    setData({
      ...data,
      ['institution_type_name']: institutiontypename,
      ['order_institution_type_name']: 20,
    });
  }, [institutiontypename]);
  ...
  useEffect(() => {
    setData({
      ...
      ['order_institution_name']: 22,
    });
  }, [institutionname]);

  useEffect(() => {
    console.log(
      'props.libraryProjectsOptionList.length: ' +
        props.libraryProjectsOptionList.length,
    );
  ...
  const onChangeStep = (formData, newStep) => {
    ...
    if (newStep == 1) {
      ...
      if (data.identifiers_id !== undefined && data.identifiers_id.length > 0)
        fields.library_identifier_list.hidden = false;
      else fields.library_identifier_list.hidden = true;
    }
  ...
  const onChangeData = (field_name, value, order) => {
    ...
    if (field_name === 'identifier_type_id') setIdentifiertype(value.value);
    ...
    if (field_name === 'institution_country_id') {
      ...
      setCountryid(countryid => {
        setData({
          ...data,
          institution_country_name: value.label,
          ['order_institution_country_name']: 21,
        });
        setData({
          ...data,
          [field_name]: value,
    ...
    if (
      field_name === 'library_identifiers_txt' &&
      value !== 0 &&
  ...
  const toggleLibraryProfile = ev => {
    ...
    if (fields.opac.hidden === false) {
      ...
      fields.subject_label.hidden = false;
      fields.showfullProfile.label = intl.formatMessage(
        wizardMessages.switchToBasicProfile,
      );
    }
  };
  ...
  const checkValidation = validation => {
    if (!validation) {
      ...
      Object.keys(steps).map(key => {
        objSteps = {
          ...objSteps,
          [key]: { active: key > currentStep ? false : true },
        };
      });
      setSteps(objSteps);
    }
      ...
      {Object.keys(currentFields).length > 0 && currentStep <= totalSteps - 1 && (
        <CustomForm
          submitCallBack={formData =>
            onChangeStep(
              formData,
              totalSteps > currentStep ? currentStep + 1 : currentStep,
            )
          }
          requestData={data ? data : null}
          onChangeData={(field_name, value, order) =>
            onChangeData(field_name, value, order)
          }
          ...
          searchOptionList={{
            institution_type_id: input =>
              dispatch(requestGetInstitutionTypeOptionList(input)),
            country_id: input => dispatch(requestGetCountriesOptionList(input)),
            subject_id: input =>
              dispatch(requestLibrarySubjectOptionList(input)),
            project_id: input => dispatch(requestGetProjectsOptionList(input)),
            institution_country_id: input =>
              dispatch(requestGetlibraryidentifierTypesOptionList(input)),
            identifier_type_id: input =>
              dispatch(requestGetIdentifierTypesOptionList(input)),
          }}
          messages={messages}
              ...
              <div className="row">
                {Object.keys(data).map((key, index) => {
                  if (!key.includes('order_')) {
                    var itemexists = itemsreport.findIndex(
                      x => x.field_name == key && x.value == data[key],
                    );
                    if (itemexists < 0) {
                      if (data[key] !== 'Select')
                        itemsreport.push({
                ...
                {itemsreport.length > 0 && sortingcount === 0
                  ? setSortingcount(sortingcount + 1)
                  : ''}

                {Object.keys(itemsreport).map((item, i) => {
                  return (
                    itemsreport[i].field_name !== 'profile_type' &&
                    itemsreport[i].field_name !== 'identifier_id' &&
                    itemsreport[i].field_name !== 'institution_type_id' &&
                    //return itemsreport[i].field_name!=='identifier_id' && itemsreport[i].field_name!=='institution_type_id'
                    itemsreport[i].field_name !== 'country_id' &&
                    itemsreport[i].field_name !== 'library_identifiers_txt' &&
                    itemsreport[i].field_name !== 'identifier_type_id' &&
                    itemsreport[i].field_name !== 'subject_id' &&
                    itemsreport[i].field_name !== 'institution_country_id' &&
                    itemsreport[i].field_name !== 'institution_id' &&
                    itemsreport[i].field_name !== 'order' &&
                    itemsreport[i].field_name !== 'backbuttonPressed' &&
                    (fields.suggested_institution_name.hidden
                      ? itemsreport[i].field_name !==
                        'suggested_institution_name'
                      : itemsreport[i].field_name !== 'institution_name') &&
                    (fields.opac.hidden
                      ? itemsreport[i].field_name !== 'opac'
                      : itemsreport[i].field_name !== '') && (
                      <div key={item} className="report_summary">
                        {
                          <div>
                            <div className="font-weight-bold">
                              {messages[itemsreport[i].field_name] &&
                                intl.formatMessage(
                                  messages[itemsreport[i].field_name],
                                )}
                            </div>
                            {/* to display all fields name*/}
                            {itemsreport[i].field_name !== 'identifiers_id' &&
                              itemsreport[i].field_name !== 'project_id' && (
